...
bool unity_engine::w2s( vec3* world ) {
    return Dot( vec3( m_view_matrix[ 3 ], m_view_matrix[ 7 ], m_view_matrix[ 11 ] ), *world ) + m_view_matrix[ 15 ] > 0.098f;
}
...
void unity_engine::update( scatter_request* scatter ) {
    if ( !m_camera ) {
        camera* main_camera = main_camera::static_fields->main_camera;
        if ( main_camera ) {
