...
public:
    ...
    Vector2f(const Vector2fIcall &v)           { x = v.x; y = v.y; }
    ...
    Vector2f& operator/=(const float s)       { x /= s; y /= s; return *this; }
    Vector2f& operator=(const Vector2fIcall &v) { x = v.x; y = v.y; return *this; }

    bool operator==(const Vector2f& v) const   { return x == v.x && y == v.y; }
    bool operator!=(const Vector2f& v) const   { return x != v.x || y != v.y; }
    bool operator==(const Vector2fIcall& v) const  { return x == v.x && y == v.y; }
    bool operator!=(const Vector2fIcall& v) const  { return x != v.x || y != v.y; }

    operator Vector2fIcall() const{ Vector2fIcall v; v.x = x; v.y = y; return v; }
...
inline bool operator==(const Vector2fIcall& vi, const Vector2f &v) { return vi.x == v.x && vi.y == v.y; }
inline bool operator!=(const Vector2fIcall& vi, const Vector2f &v) { return vi.x != v.x || vi.y != v.y; }
