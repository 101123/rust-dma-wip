...
// warning:
// CeilfToIntPos(1.f) returns 2
// CeilfToIntPos(256.f) returns 257
// etc.. (see conversation from https://unity.slack.com/archives/C06TQ0QMQ/p1447258079000524 )
...
inline float SignedWordToNormalized(int p)
{
    return (float)p * (1.0f / 32767.0f);
}

inline float UnsignedWordToNormalized(int p)
{
    return (float)p * (1.0f / 65535.0f);
}

// Convert float [-1...1] to unsigned word [0...65535]
inline int NormalizedToUnsignedWord(float f)
{
    f = FloatClamp(f, -1.0f, 1.0f);
    return RoundfToInt(f * 65535.0f);
}
...
inline bool IsHalfNAN(UInt16 value)
{
    // Returns true if the Half value represents NaN float
...
// f32: sign1 | exponent8 | fraction23
// f16: sign1 | exponent5 | fraction10
// f11:         exponent5 | fraction6
// f10:         exponent5 | fraction5
...
/*                               |------------------exponent-----------------|  |----fraction---|  */
#define FP32_TO_FP11_MAGIC(fi)  ((((((fi)&0x7f800000)-0x38000000)>>17)&0x7C0) | (((fi)>>17)&0x3F))
/*                               |------------exponent-----------|   |-----fraction----|  */
#define FP11_TO_FP32_MAGIC(fi)  ((((((fi)>>6)&0x1F)+(127-15))<<23) | (((fi)&0x3F) << 17))
/*                               |------------------exponent-----------------|  |----fraction---|  */
#define FP32_TO_FP10_MAGIC(fi)  ((((((fi)&0x7f800000)-0x38000000)>>18)&0x3E0) | (((fi)>>18)&0x3F))
/*                               |------------exponent-----------|   |-----fraction----|  */
#define FP10_TO_FP32_MAGIC(fi)  ((((((fi)>>5)&0x1F)+(127-15))<<23) | (((fi)&0x1F) << 18))
...
inline float ConvertFP16ToFP32(UInt16 val_)
{
    ...
    fpbits.u32 = (val & 0x7FFF) != 0 ? FP16_TO_FP32_MAGIC(val) : 0;
    return fpbits.fp32;
}
...
// we need f11/f10 only as parts of some combined structure, like R11.G11.B10 color
// so using u32 for f11/f10 is the best options, as we can recombine these directly

inline UInt32 ConvertFP32ToFP11(float val)
{
    fp::internal::FP32Bits fpbits;
    fpbits.fp32 = val;

    const UInt32 fi = fpbits.u32;
    return fi != 0 ? FP32_TO_FP11_MAGIC(fi) : 0;
}

inline float ConvertFP11ToFP32(UInt32 val)
{
    fp::internal::FP32Bits fpbits;
    fpbits.u32 = (val & 0x7FF) != 0 ? FP11_TO_FP32_MAGIC(val) : 0;
    return fpbits.fp32;
}

inline UInt32 ConvertFP32ToFP10(float val)
{
    fp::internal::FP32Bits fpbits;
    fpbits.fp32 = val;

    const UInt32 fi = fpbits.u32;
    return fi != 0 ? FP32_TO_FP10_MAGIC(fi) : 0;
}

inline float ConvertFP10ToFP32(UInt32 val)
{
    fp::internal::FP32Bits fpbits;
    fpbits.u32 = (val & 0x3FF) != 0 ? FP10_TO_FP32_MAGIC(val) : 0;
    return fpbits.fp32;
}

#undef FP16_TO_FP32_MAGIC
#undef FP32_TO_FP16_MAGIC
#undef FP32_TO_FP11_MAGIC
#undef FP11_TO_FP32_MAGIC
#undef FP32_TO_FP10_MAGIC
#undef FP10_TO_FP32_MAGIC
...
// Fabian Giesen's public domain half_to_float_fast4
inline float HalfToFloat(UInt16 src)
{
    static const UnionTuple<UInt32, float> magic = { 113 << 23 };
    static const UInt32 shiftedExp = 0x7c00 << 13; // exponent mask after shift
    UnionTuple<UInt32, float> o;

    o.first = (src & 0x7fff) << 13; // exponent/mantissa bits
    UInt32 exponent = shiftedExp & o.first; // just the exponent
    o.first += (127 - 15) << 23; // exponent adjust

    // handle exponent special cases
    if (exponent == shiftedExp) // Inf/NaN?
    {
        o.first += (128 - 16) << 23; // extra exp adjust
    }
    else if (exponent == 0) // Zero/Denormal?
    {
        o.first += 1 << 23; // extra exp adjust
        o.second -= magic.second; // renormalize
    }

    o.first |= (src & 0x8000) << 16; // sign bit
    return o.second;
}
...
{
    return (int)Pow(2, Roundf(fmaxf(Log2(v), 0.0f)));
}
