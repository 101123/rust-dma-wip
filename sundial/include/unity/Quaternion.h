...
Vector3f QuaternionToEuler(const Quaternionf& quat, math::RotationOrder order = math::kOrderUnityDefault);
...
inline Quaternionf NormalizeSafeIfUnnormalized(const Quaternionf& q)
{
    ...
    if (Abs(mag - 1.0f) < Vector3f::epsilon)
        return q;
